...
export default {
    ...
    methods: {
        ...
        submit() {
            ...
            if (this.$route.name == 'UserKepalaSekolah-edit-id') {
                ...
                this.updateUserData(data).then((this.$router.push({name: 'UserKepalaSekolah'})))
            } else {
                //SELAIN ITU MAKA FUNGSI ADD DATA YANG AKAN DIJALAKAN
                this.storeUsersData(this.users).then(() => this.$router.push({name: 'UserKepalaSekolah'}))
            }
        }
    },
