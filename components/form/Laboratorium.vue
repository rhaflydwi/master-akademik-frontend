...
export default {
        created() {
            if (this.$route.name == 'laboratorium-edit-id') {
                this.laboratorium = {
       ...
       ...mapActions('laboratoriums', ['storeLaboratoriumData', 'updateLaboratoriumsData']),
        submit() {
            if (this.$route.name == 'laboratorium-edit-id') {
                ...
                this.updateLaboratoriumsData(data).then((this.$router.push({name: 'laboratorium'})))
            } else {
                this.storeLaboratoriumData(this.laboratorium).then(() => this.$router.push({name: 'laboratorium'}))
                    }
                }
            }
}
