...
export default {
    layout: 'DefaultAdmin',
    //KETIKA PAGE DILOAD, MAKA
     //FUNGSI INI AKAN DIJALANKAN SECARA OTOMATIS
    //DIMANA KITA MEMANGGIL FUNGSI GETUSERSDATA DARI STORE USER
    async asyncData({store}) {
        await Promise.all([
            store.dispatch('UserKepalasekolah/getUsersData')
        ])
        return
    },
    data() {
    ...
    methods: {
        ...mapActions('UserKepalasekolah', ['getUsersData', 'destroyUsersData']),
        ...mapMutations('UserKepalasekolah', ['SET_PAGE']),
