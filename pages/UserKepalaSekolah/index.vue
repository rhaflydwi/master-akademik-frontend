                ...
                <div class="col-md-12">
                    <b-table striped hover :items="users.data" :fields="fields" show-empty responsive>
                        ...
                        <template v-slot:cell(actions)="row">
                            ...
                            <nuxt-link :to="{name: 'UserKepalaSekolah-edit-id', params: {id: row.item.id}}" :key="'edit'+row.index" class="btn btn-warning btn-sm" style="margin:18px">Edit</nuxt-link>
                            <!-- TOMBOL HAPUS AKAN MEMBUKA MODAL KONFIRMASI --> 
                            <button class="btn btn-danger btn-sm" style="margin:10px" @click="openDeleteModal(row)">Delete</button>
                        </template>
                        <!-- MENAMPILKAN TOMBOL EDIT DAN HAPUS --> 
...
export default {
    ...
    data() {
        return {
            ...
            fields: ['name','address', 'email', 'phone_number', 'photo', 'actions'],
            items: [],
            deleteModal: false, 
