        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title m-0 font-weight-bold text-primary"><i class="fa fa-file" aria-hidden="true"></i>
                    Data Nilai Kelas VIII
                    <nuxt-link class="btn btn-primary float-right btn-sm" :to="{name: 'nilaisiswaguruall-add'}">Add New</nuxt-link>
                </h2>
            </div>
                ...
                <div class="col-md-12">
                    <b-table striped hover :items="nilai.data" :fields="fields" show-empty responsive>
                        ...
                        <template v-slot:cell(actions)="row">
                            ...
                            <nuxt-link :to="{name: 'nilaisiswaguruall-edit-id', params: {id: row.item.id}}" :key="'edit'+row.index" class="btn btn-warning btn-sm">Edit</nuxt-link>
...
export default {
    ...
    async asyncData({store}) {
        await Promise.all([
            store.dispatch('nilaisiswa/getNilaiSiswaViiiData')
        ])
        return
    },
    data() {
        return {
            ...
            fields: ['name','kelas_teks', 'mata_pelajaran_teks','nilai_teks', 'actions'], 
            items: [],
            deleteModal: false, 
    ...
    watch: {
        ...
        page() {
            ...
            this.getNilaiSiswaViiiData(this.search)
        }  
    },
    methods: {
        ...mapActions('nilaisiswa', ['getNilaiSiswaViiiData', 'destroyNilaiData']),
