        ...
        <div class="card">
            <div class="card-header">
                <h2 class="card-title m-0 font-weight-bold text-primary"><i class="fa fa-calendar" aria-hidden="true"></i>
                    Jadwal Siswa Kelas VII
                    <nuxt-link class="btn btn-primary float-right btn-sm" :to="{name: 'jadwalsiswa-add'}">Tambah Jadwal</nuxt-link>
...
export default {
    ...
    async asyncData({store}) {
        await Promise.all([
            store.dispatch('JadwalSiswa/getJadwalSiswaViiData')
        ])
        return
    },
    data() {
        return {
            ...
            fields: ['hari','jam', 'ruang', 'mata_pelajaran', 'kelas', 'guru_pengampu', 'actions'], 
            items: [],
            deleteModal: false, 
